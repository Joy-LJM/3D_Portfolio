...
const socials = [
  ...
  { label: 'LinkedIn', handle: 'https://www.linkedin.com/in/jiamin-luo/', href: 'https://linkedin.com' },
]
</script>
    ...
    <div class="container">
      ...
      <div class="contact-layout">
          ...
          <ul class="contact-socials" role="list">
            <li v-for="s in socials" :key="s.label">
              <a :href="s.href" class="social-row" target="_blank" rel="noopener noreferrer">
                ...
                <svg class="social-arrow" width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
        ...
        <div class="contact-right">
          <form v-if="!sent" class="contact-form" @submit.prevent="onSubmit" novalidate>
            <div class="field">
          ...
          <div v-else class="sent-state" role="status">
            ...
            <p class="sent-message">Message received. I'll reply within one business day.</p>
          </div>
        </div>
      </div>
    </div>
...
.contact-socials { list-style: none; display: flex; flex-direction: column; gap: 0; }
...
.contact-socials li:first-child .social-row { border-top: 1px solid var(--border); }
.social-row:hover { padding-left: 8px; }

.social-label {
  font-family: var(--font-mono);
  font-size: 10px; letter-spacing: 0.18em; text-transform: uppercase;
  color: var(--accent); min-width: 68px;
}
.social-handle {
  font-family: var(--font-mono);
...
.social-arrow { color: var(--muted); opacity: 0; transition: opacity 0.3s; }
...
.form-btn:hover { background: var(--text); transform: translateX(4px); }
...
@keyframes pulse {
  0%,100% { box-shadow: 0 0 0 0 rgba(123,47,190,0.5); }
